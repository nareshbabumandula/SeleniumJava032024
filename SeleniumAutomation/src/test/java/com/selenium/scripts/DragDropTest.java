...
public class DragDropTest {
	...
	public void draganddropMethods() throws InterruptedException {
		...
		try {
			...
			Thread.sleep(2000);
			action.contextClick(driver.findElement(By.linkText("Development"))).perform(); // Right Click
			...
			test.log(LogStatus.FAIL, "Failed to peform drag and drop since - " + message);
			System.out.println("Failed to perform drag and drop since - " + message);
			Assert.assertEquals(bFlag, true, "Failed to perform action class methods since : " + message);
		}finally {
			if (bFlag) {
